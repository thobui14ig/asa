...
import SiderBar from "./SiderBar-New";
interface MENU {
  key: string;
  icon: string | null;
  children: any;
...
const Sidebar = () => {
  ...
  const createMenu = (data: any) => {
    return data.map((item: any) => {
      let projects = item.projects.map((project: any) =>
        getItem(project.name, project._id)
      );
      const showMore = {
  ...
  const createShowmoreShowless = (e: any, showName: string) => {
    ...
    let projects = [...projectSelected.projects].map((project: any) => {
      return getItem(project.name, project._id);
    });
